...
from handlers.certificate_generator import handle_certificate_generation
from handlers.report_generator import generate_report_async
...
class App(TkinterDnD.Tk if TkinterDnD else ctk.CTk):
    ...
    # ── GERAR PDF (delegado para handlers) ─────────────────────────────
    def generate(self):
        if self.document_type_var.get() == "Certificado de treinamento":
            handle_certificate_generation(
                persist_certificate_fields=self._persist_certificate_fields,
        ...
        generate_report_async(
            ...
            fotos=self.fotos,
            config_data=self.config_data,
            foto_cols_var=self.foto_cols_var,
            foto_h_var=self.foto_h_var,
            watermark_opacity_var=self.watermark_opacity_var,
            watermark_scale_var=self.watermark_scale_var,
            watermark_mode_var=self.watermark_mode_var,
            signature_var=self.signature_var,
            get_watermark_random_count=self._get_watermark_random_count,
            get_section_offsets_cm=self._get_section_offsets_cm,
            get_horarios_from_ui=self._get_horarios_from_ui,
            pick_pdf_save_dir=self._pick_pdf_save_dir,
            remember_pdf_save_dir=self._remember_pdf_save_dir,
            set_status=self._set_status,
            hide_open_pdf_button=self._btn_abrir_pdf.pack_forget,
            on_success=lambda save: self.after(0, lambda: self._on_generate_success(save)),
            on_error=lambda err: self.after(0, lambda: self._on_generate_error(err)),
            validar_sections_fn=validar_sections,
            gerar_pdf_fn=gerar_pdf,
        )
